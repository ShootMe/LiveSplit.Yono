        ...
        public void InitializeSettings(XmlNode node) {
            ...
            foreach (XmlNode splitNode in splitNodes) {
                ...
                if (splitValues.Length == 2) {
                    ...
                    if (!addedAutoStart) {
                        Settings.Autosplits.Add(new Split() { Type = SplitType.GameStart });
                        addedAutoStart = true;
                    }
                    string key = splitValues[0];
                    if (Enum.TryParse<SplitArea>(key, out SplitArea area)) {
                        Settings.Autosplits.Add(new Split() { Type = SplitType.AreaEnter, Value = key });
                    } else if (key == "HealthToken") {
                        Settings.Autosplits.Add(new Split() { Type = SplitType.HealthToken });
                    } else if (key == "EndGame") {
                        Settings.Autosplits.Add(new Split() { Type = SplitType.GameEnd });
                    }
                }
            }
